...
class SentenceClassifier(object):
    def __init__(self, hidden_size=10, vocab_size=42791, embedding_size=50, embedding_matrix=None):
        ...
        with tf.variable_scope("embeddings"):
            if embedding_matrix == None:
                self.W_embeddings = tf.get_variable(shape=[vocab_size, embedding_size], \
                                               ...
                                               name="W_embeddings", dtype=tf.float64)
            else:
            ...
            sentence_batch_mask = tf.cast(tf.reshape(batch_mask, [-1, 1, 1]), tf.float64)
            ...
            masked_sentence_embeddings = tf.mul(sentence_embeddings, tf.cast(tf.expand_dims(sentence_mask, -1), tf.float64))
            ...
            question_mask_float = tf.expand_dims(tf.cast(question_mask, tf.float64), -1)
        ...
        with tf.variable_scope("similarity-scoring"):
            ...
            #
            # # softmax
            # numerator = tf.exp(tf.sub(dot_prod, tf.expand_dims(tf.reduce_max(dot_prod, 1), -1))) * tf.cast(batch_mask, tf.float64)
            ...
            #
            # masked_pos_cos_sim = tf.sub(tf.add(cosine_similarity, 1), tf.cast(batch_mask < 1, tf.float64))
            ...
            alpha_weights, attend_result = attention(attending=question_cbow, attended=doc_sentences, \
                time_mask=tf.cast(batch_mask, tf.float64))
        ...
        with tf.variable_scope("prediction"):
            one_hot_labels = tf.one_hot(labels, max_sent_per_doc, dtype=tf.float64)
            ...
            correct_vector = tf.cast(tf.equal(labels, tf.cast(tf.argmax(probabilities, 1), tf.int32)), \
                tf.float64, name="correct_vector")

...
class AbstractTagger(object):
    ...
    def evaluate(self, predictions, output):
        ...
        for pred, inst, gold in zip(predictions, self.test_X_in, self.test_X_org):
            ...
            for word, p in zip(gold, predicted_unpadded):
                if output:
                    print("%s\t%s\t" % (word, p))
                total += 1
            if output:
                print("")
        # print("correct %s total: %s accuracy: %s" % (correct, total, correct / total))
...
class SequenceTagger(AbstractTagger):
    ...
    def read_data(self, trainfile, testfile, dev=None, freeze=False):
        ...
        print("max_sentence_len:", self.max_sentence_len)
